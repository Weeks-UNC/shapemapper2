...
class Pipeline(Component):
    """
    Note: A Pipeline is still a Component, so Pipelines can be reused as
    pieces of larger workflows.
    ...
    def get_error_message(self,
        ...
        for c in failed_components:
            msg += c.get_error(verbose=verbose)
        return msg
        ...
        while True:
            ...
            while True:
                if timeout is not None:
                    if timer() - start_time > timeout:
                        if not quiet:
                            # FIXME: show timeout errors in test runners
                ...
                if break_flag:
                    ...
                    if success:
                        ...
                        sleep(0.5) # wait a bit longer on failure so other procs can terminate, write errors
                ...
                if ( progmon is not None
                    ...
                    if o is not None:
                        ...
                        if len(s) > 1:
                            ...
                            pad = "".join([' ' for x in range(term_width - len(s) - level)])
            ...
            if not quiet:
                if success:
                    ...
                    s = self.get_error_message(run_group)
                sys.stdout.write(s)
            if not success:
                self.clean_up()
                break
...
def run_fail_test(pipeline=None,
    ...
    # TODO: allow use of existing bowtie index (this should dramatically speed up overall test execution)
    passed = False
    success = 1
    err = ""
    try:
        ...
        success = pipeline.run(timeout=30,
                               quiet=True,
                               latency=0.1) # fails to detect bowtie2 exit with 0, 0.001, 0.01, not 0.1
                                            # FIXME: running with a fast loop on test 0.1.8, proc.returncode changes !? from 0 to 134 somehow between calls but haven't tracked down where or how

        if not success:
            err += pipeline.get_error_message(verbose=True).strip()
            fail_mod_id = None

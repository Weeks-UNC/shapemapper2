...
reactivity profile suitable for VARNA or Ribosketch import
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--varna", required=False, type=str)
    parser.add_argument("--ribosketch", required=False, type=str)    

    p = parser.parse_args(sys.argv[1:])

    seq, shape, stderr = load_tab(p.infile)

    truncated = truncate_profile(shape)
    ...
    if p.varna:
        write_simple(truncated, p.varna)
    if p.ribosketch:
        write_simple(truncated, p.ribosketch)

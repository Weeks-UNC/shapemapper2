...
namespace mutation_parser { namespace detail {
    ...
    parseCIGAR(const std::string &cigar_string){
        std::vector<BamTools::CigarOp> cigar_data;
        std::vector<std::string> fields;
        ...
        for (int i=0; i < fields.size(); i=i+2){
            ...
            cigar_data.push_back(BamTools::CigarOp(op,len));
        }
        return cigar_data;
    }
    ...
    locateMutations(const int pos,
                    ...
                    const std::vector<BAM::CigarOp> &cigar_data,
        ...
        for (co = 0; co < cigar_data.size(); ++co) {
            char c_type = cigar_data[co].Type;
                ...
                case ('M'): {
                    ...
                    while (mo < md_data.size() and
                        ...
                        if (tmp_op == MD_MATCH) {
                            ...
                            assoc_md[co].append(
                                    MD_op_names[tmp_op] + ' ' + std::to_string(overlap_length) + ' ' + query_seq_overlap +
                                    ' ');
        ...
        /*for (int i = 0; i < cigar_data.size(); ++i) {
            ...
            std::cout << l << cigar_data[i].Length << cigar_data[i].Type << r << "-[";
            std::cout << assoc_md[i] << "] ";
        }
        //std::cout <<'\n'<< target;
        std::cout << '\n';*/
    ...
    std::vector<Mutation>
    adjustAmbiguousMutations(const int pos,
        ...
        std::vector<std::vector<int>> appended_target_indices(mutations.size()); // helper array to hold temporary information about
     ...
     *         valid placements (see adjustMutations()).
     */
    ...
    parseMutations(const int left_target_pos,
        ...
        // TODO: option not to perform this calculation?
        std::vector<Mutation> adjusted_mutations = adjustAmbiguousMutations(left_target_pos,
                                                                            local_target_seq,
                                                                            local_target_qual,
                                                                            aligned_query_seq,
                                                                            aligned_query_qual,
        ...
        return boost::make_tuple(left_target_pos,
                                 right_target_pos,
                                 local_target_seq,
                                 local_target_qual,
                                 adjusted_mutations);
